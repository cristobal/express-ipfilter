...
function processReq(req, settings, cache) {
    ...
    if (!has(cache, 'processReq')) {
        ...
        cache.processReq = {};
        cache.processReq.including = fn(settings.including);
        cache.processReq.excluding = fn(settings.excluding);
    }

    var including = cache.processReq.including;
    var excluding = cache.processReq.excluding;
    var url = req.url;
    ...
    // check if url is in the including list to process
    if (including.length > 0) {
        return find(including, test) !== void 0;
    }

    // check if the url is in the excluding list to process
    if (excluding.length > 0) {
        return find(excluding, test) === void 0;
    }
 ...
 *  - 'including' routes that should be included as part of the ip filtering, this takes precedence over excluding.
 *                and will only process the routes that matches the including list.
 *
